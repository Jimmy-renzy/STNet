...
from keras.layers.core import Dense, Lambda, PCLDense, RanPCLDense
...
class DNNDecoder():
    def __init__(self, alpha=1.8, GSNR = 1, addPCL=False, addRanPCL=False, use_bias=True):
        self.k = 8
        self.N = 16
        ...
        self.model, self.modulator_layers, self.noise, self.decoder_layers, self.decoder = self.build_Decoder(addPCL, addRanPCL, use_bias)
    ...
    def build_Decoder(self, addPCL, addRanPCL, use_bias):
        # Define modulator
        ...
        # Define decoder
        decoder_layers = [Dense(self.design[0], activation='relu', input_shape=(self.N,), use_bias=use_bias)]
        if addPCL == True:
            print("addPCL == True")
            decoder_layers.append(PCLDense(self.design[1], activation='relu', use_bias=use_bias))
        elif addRanPCL == True:
            print("addRanPCL == True")
            decoder_layers.append(RanPCLDense(self.design[1], activation='relu', use_bias=use_bias))
        else:
            print("Fully Connected Layer")
            decoder_layers.append(Dense(self.design[1], activation='relu', use_bias=use_bias))
        decoder_layers.append(Dense(self.design[2], activation='relu', use_bias=use_bias))
...
if __name__ == '__main__':
    DnnDecoder = DNNDecoder(1.2, 1, addPCL=False, addRanPCL=False, use_bias=True)
    DnnDecoder.train(2 ** 18, 256, 1, 1)
    # DnnDecoder.model.save_weights("Weights/DNN_weights/DNN_weights_218_alpha1.2")
    ber = DnnDecoder.test(1.2, 0, 9, 10, 100, 500000)
    print("ber:", ber)
